...
export default function Pricing({ user, products, subscription }: Props) {
  ...
  const handleStripeCheckout = async (price: Tables<'prices'> & { type: 'one_time' | 'recurring' }) => {
    ...
    <div className="max-w-6xl px-4 py-8 mx-auto sm:py-24 sm:px-6 lg:px-8">
      <div className="sm:flex sm:flex-col sm:align-center">
        ...
        <p className="max-w-2xl m-auto mt-5 text-xl sm:text-center sm:text-2xl">
          Let&apos;s get you set up with a subscription that&apos;s a ... blast... and works best for your goals!
        </p>
      ...
      <div className="mt-12 space-y-4 sm:mt-16 sm:space-y-0 sm:grid sm:grid-cols-2 sm:gap-6 lg:max-w-4xl lg:mx-auto xl:max-w-none xl:mx-0 xl:grid-cols-3">
        {products.map((product) => {
          const price = product.prices.find(
            (price) => price.type === (billingInterval === 'one_time' ? 'one_time' : 'recurring') &&
                       (billingInterval === 'one_time' || price.interval === billingInterval)
          );
