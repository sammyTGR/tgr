...
export default function FirearmsChecklist() {
              ...
              prevData.map((item) =>
                item.id === changedFirearm.id
                  ? {
                      ...
                      highlight: getHighlightColor(changedFirearm.rental_notes || ''),
                    }
                  : item
              )
            );
  ...
  const fetchFirearmsMaintenanceData = useCallback(async () => {
    ...
    if (verificationsError) {
      console.error('Error fetching verification data:', verificationsError.message);
            ...
            prevData.map((item) =>
              item.id === newVerification.firearm_id
                ? {
                    ...
                    morning_checked:
                      newVerification.verification_time === 'morning' ? true : item.morning_checked,
                    evening_checked:
                      newVerification.verification_time === 'evening' ? true : item.evening_checked,
                  }
                : item
            )
          );
  ...
  const handleNotesChange = (id: number, notes: string) => {
    setData((prevData) => prevData.map((item) => (item.id === id ? { ...item, notes } : item)));
  };
  ...
  const handleSubmitChecklist = async () => {
    ...
    try {
      ...
      const firearmsRentedOut = data.some((item) => item.notes === 'Currently Rented Out');
      ...
      const { error } = await supabase
        ...
        .in(
          'id',
          data.filter((item) => item.notes === 'Verified').map((item) => item.id)
        );
      ...
      const updatedData = data.map((firearm) =>
        firearm.notes === 'Verified' ? { ...firearm, notes: '', verified_status: '' } : firearm
      );
        ...
        <section className="flex-1 flex flex-col space-y-4 p-4">
          ...
          <div className="flex items-center space-y-2">
            <div className="flex space-x-2">
              ...
              >
                {submittingChecklist ? 'Submitting Checklist...' : 'Submit Checklist'}
              </Button>
              ...
              {['super admin', 'dev'].includes(userRole || '') && (
                <>
                  ...
                  >
                    {submittingBulkVerify ? 'Bulk Verifying Firearms...' : 'Bulk Verify'}
                  </Button>
