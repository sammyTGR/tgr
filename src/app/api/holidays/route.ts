  ...
  try {
    ...
    if (referenceSchedules && referenceSchedules.length > 0) {
      ...
      for (const emp of activeEmployees || []) {
        const refSchedule = referenceSchedules.find((ref) => ref.employee_id === emp.employee_id);
        ...
        const { error: scheduleError } = await supabase.from('schedules').upsert(scheduleData, {
