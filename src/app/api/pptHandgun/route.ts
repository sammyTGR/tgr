...
export async function POST(request: Request) {
  ...
  try {
    ...
    const dbData = {
      ...
      is_us_citizen: formData.isUsCitizen?.toLowerCase() === 'yes' ? 'yes' : 'no',
      ...
      seller_is_us_citizen: formData.sellerIsUsCitizen?.toLowerCase() === 'yes' ? 'yes' : 'no',
    ...
    if (dbData.firearms_q1 && !['yes', 'no', 'n/a'].includes(dbData.firearms_q1)) {
