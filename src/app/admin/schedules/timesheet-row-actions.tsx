...
export function TimesheetRowActions({
  ...
  const formatTimeForInput = useCallback((timeString: string | null): string => {
    ...
    try {
      ...
      console.error('Error formatting time for input:', error, 'for time value:', timeString);
      return '';
    }
  }, []);

  const formatTimeForDisplay = useCallback((timeString: string | null): string => {
    if (!timeString) return '';
    try {
      ...
      console.error('Error formatting time for display:', error, 'for time value:', timeString);
      return '';
    }
  }, []);
  ...
  const convertToUTCTime = (time: string): string | null => {
    ...
    try {
      ...
      console.error('Error converting time to UTC:', error, 'for time value:', time);
      return null;
    }
  };
  ...
  const applyChanges = async () => {
    ...
    if (Object.keys(updates).length > 0) {
      ...
      if (error) {
        ...
        const currentExpandedState = queryClient.getQueryData(['timesheetExpandedState']);
        const currentSortingState = queryClient.getQueryData(['timesheetSortingState']);
        ...
        queryClient.setQueryData(['timesheetExpandedState'], currentExpandedState);
        queryClient.setQueryData(['timesheetSortingState'], currentSortingState);
    ...
    <DropdownMenu open={isOpen} onOpenChange={handleDropdownOpenChange}>
      ...
      <DropdownMenuContent align="end" className="w-[400px] p-2">
        <div className="flex items-center space-x-2 mb-2">
          ...
          <Button size="sm" variant="ghost" onClick={() => clearField('start_time')}>
            <X className="h-4 w-4" />
        ...
        <div className="flex items-center space-x-2 mb-2">
          ...
          <Button size="sm" variant="ghost" onClick={() => clearField('lunch_start')}>
            <X className="h-4 w-4" />
        ...
        <div className="flex items-center space-x-2 mb-2">
          ...
          <Button size="sm" variant="ghost" onClick={() => clearField('lunch_end')}>
            <X className="h-4 w-4" />
        ...
        <div className="flex items-center space-x-2 mb-2">
          ...
          <Button size="sm" variant="ghost" onClick={() => clearField('end_time')}>
            <X className="h-4 w-4" />
          ...
          <AlertDialogContent>
            <AlertDialogHeader>
              ...
              <AlertDialogDescription>
                ...
                {timesheet.employee_name} on {new Date(timesheet.event_date!).toLocaleDateString()}.
              </AlertDialogDescription>
            </AlertDialogHeader>
            <AlertDialogFooter>
