...
function AdminDashboardContent({ handleTabHover }: { handleTabHover?: (tab: string) => void }) {
  ...
  // Add delete suggestion mutation
  const deleteSuggestionMutation = useMutation({
    mutationFn: async (suggestionId: number) => {
      const { error } = await supabase.from('employee_suggestions').delete().eq('id', suggestionId);
  ...
  // Add this near your other queries
  const { data: suggestionToDelete } = useQuery({
    queryKey: ['suggestionToDelete'],
    queryFn: () => null as Suggestion | null,
    staleTime: Infinity,
  });

  // Add this near your other mutations
  const suggestionToDeleteMutation = useMutation({
    mutationFn: (suggestion: Suggestion | null) => Promise.resolve(suggestion),
    onSuccess: (suggestion) => {
      queryClient.setQueryData(['suggestionToDelete'], suggestion);
    },
  });
                  ...
                  <div className="col-span-full overflow-hidden mt-2">
                    <ScrollArea
                      className={classNames(
                        styles.noScroll,
                        'h-[calc(100vh-25rem)] w-full overflow-hidden relative'
                      )}
                    >
                                      ...
                                      {suggestion.is_read && (
                                        <Dialog>
                                          ...
                                          <DialogContent className="sm:max-w-[425px]">
                                            <DialogHeader>
                                              <DialogTitle>Suggestion Reply</DialogTitle>
                                            </DialogHeader>
                                            <div className="grid gap-4 py-4">
                                              <div className="space-y-2">
                                                <h4 className="font-medium">Original Suggestion</h4>
                                                <p className="text-sm border p-2 rounded bg-muted/50">
                                                  {suggestion.suggestion}
                                                </p>
                                              </div>
                                              <div className="space-y-2">
                                                <h4 className="font-medium">Current Reply</h4>
                                                <Textarea
                                                  value={
                                                    // Initialize with current reply text
                                                  ...
                                                  onChange={(e) => {
                                                    // Update reply text in the query cache
                                                    queryClient.setQueryData(
                                                      ['replyText', suggestion.id],
                                                      e.target.value
                                                    );
                                                  }}
                                              ...
                                              <div className="text-xs text-gray-500">
                                                <p>
                                                  Originally replied by: {suggestion.replied_by}
                                                </p>
                                                <p>
                                                  Replied at:{' '}
                                                  {new Date(
                                            ...
                                            <DialogFooter>
                                              <Button
                                                onClick={() => {
                                                  const replyText = queryClient.getQueryData([
                                                    'replyText',
                                                    suggestion.id,
                                                  ]) as string;
                                                  if (replyText && replyText !== suggestion.reply) {
                                                    handleReply({
                                                      suggestion,
                                                      replyText,
                                                    }).then(() => {
                                                      toast.success(
                                                        'Reply updated and email notification sent'
                                                      );
                                                    });
                                                  }
                                                }}
                      ...
                      <div className="space-y-4">
                        <label className="flex items-center text-center gap-2 p-2 rounded-md cursor-pointer border border-gray-300 hover:bg-gray-100 dark:hover:bg-gray-700 w-full">
                          ...
                          <span className="text-center items-center">
                            {detailedSalesFileData?.fileName || 'Select Sales File'}
                          </span>
                        </label>
                        <Button
                          variant="outline"
