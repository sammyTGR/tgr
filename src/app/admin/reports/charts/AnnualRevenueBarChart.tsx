...
export default function AnnualRevenueBarChart() {
  ...
  const { data: historicalRevenueData = [] } = useQuery<RevenueData[]>({
    ...
    queryFn: async () => {
      ...
      if (!response.ok) throw new Error('Failed to fetch historical revenue data');
      return response.json();
    },
  });
    ...
    <Card>
      ...
      <CardContent className="p-4">
        <ChartContainer config={chartConfig} className="min-h-[200px] max-h-[500px] w-full">
          <BarChart data={combinedData} margin={{ top: 10, right: 10, left: 40, bottom: 40 }}>
                    ...
                    <div className="rounded-lg border border-border/50 bg-background p-2 shadow-xl">
                      ...
                      <div className="space-y-1">
                        <p className="text-muted-foreground">
                          2023 {chartConfig[activeChart as keyof typeof chartConfig].label}
                        </p>
                        <p className="font-mono font-medium">
                          {currencyFormatter.format(historicalValue)}
                        </p>
                        <p className="text-muted-foreground">
                          2024 {chartConfig[activeChart as keyof typeof chartConfig].label}
                        </p>
                        <p className="font-mono font-medium">
                          {currencyFormatter.format(currentValue)}
                        </p>
                        <p className="text-muted-foreground">
                          2025 {chartConfig[activeChart as keyof typeof chartConfig].label}
                        </p>
                        <p className="font-mono font-medium">
                        ...
                        <div className="mt-2 pt-2 border-t border-border/50">
                          <p
                            className={`font-medium ${
                              yearOverYearDiff > 0 ? 'text-green-500' : 'text-red-500'
                            }`}
                          >
                          ...
                          <p
                            className={`font-medium ${
                              futureYearDiff > 0 ? 'text-green-500' : 'text-red-500'
                            }`}
                          >
            ...
            />
            <Bar
              dataKey={`historical${activeChart.charAt(0).toUpperCase() + activeChart.slice(1)}`}
              name="2023"
              radius={[4, 4, 0, 0]}
              maxBarSize={40}
              fill={chartConfig[activeChart as keyof typeof chartConfig].historicalColor}
            />
            <Bar
              dataKey={activeChart}
              name="2024"
            ...
            />
            <Bar
              dataKey={`future${activeChart.charAt(0).toUpperCase() + activeChart.slice(1)}`}
              name="2025"
              radius={[4, 4, 0, 0]}
              maxBarSize={40}
              fill={chartConfig[activeChart as keyof typeof chartConfig].futureColor}
            />
          </BarChart>
        </ChartContainer>
      </CardContent>
