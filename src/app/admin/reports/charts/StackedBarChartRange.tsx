...
interface ChartData {
  ...
  Date: string; // Keep this field
  last_name: string;
  Total: number;
  TotalMinusExclusions: number;
  [key: string]: any;
}

interface StackedBarChartRangeProps {
...
const StackedBarChartRange: React.FC<StackedBarChartRangeProps> = ({
  ...
  const processData = (data: any[], currentView: ChartView): ProcessedData => {
    ...
    data.forEach((item) => {
      const lanid = item.lanid; // Changed from Lanid
      ...
      if (!excludeCategoriesFromChart.includes(category)) {
        ...
        if (!existingEntry) {
          existingEntry = {
            Lanid: lanid, // Uppercase to match interface
            Date: new Date().toISOString(), // Add default date if needed
