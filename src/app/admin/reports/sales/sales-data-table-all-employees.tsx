...
const SalesDataTableAllEmployees: React.FC = () => {
  ...
  const [selectedEmployees, setSelectedEmployees] = React.useState<string[]>(['all']);
  ...
  const filteredEmployees = React.useMemo(() => {
    ...
    return validEmployees.filter((employee) => {
      const fullName = `${employee.name || ''} ${employee.last_name || ''}`.toLowerCase().trim();
  ...
  const showEmptyState = !salesQuery.data?.data?.length && !salesQuery.isLoading;
                  ...
                  {selectedEmployees.map((id) => {
                    ...
                    return (
                      <Badge key={id} variant="secondary" className="mr-1">
                        {`${emp?.name || ''} ${emp?.last_name || ''}`.trim() || 'Unknown'}
                      </Badge>
                    );
                  })}
                </div>
