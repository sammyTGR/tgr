...
export function SalesTableToolbar<TData>({
  ...
  const isFiltered = lanidFilter.length > 0 || invoiceFilter.length > 0 || dateFilter.length > 0;
  ...
  const handleFilterChange = (columnId: string, value: string) => {
    ...
    const newFilters = table
      .getState()
      .columnFilters.map((filter) => (filter.id === columnId ? { id: columnId, value } : filter));
    ...
    <div className="flex items-center justify-between">
      <div className="flex flex-1 items-center space-x-2 p-1">
        ...
        <Input
          ...
          value={invoiceFilter}
          onChange={(event) => handleFilterChange('Invoice', event.target.value)}
          className="h-8 w-[150px] lg:w-[250px]"
        />
        <Input
