...
const AdminReportsPage = () => {
  ...
  const { data: vacationTimeData = [] } = useQuery({
    ...
    queryFn: async () => {
      const { data, error } = await supabase.rpc('get_all_employee_vacation_time_usage');
      if (error) throw error;
  ...
  const filteredVacationData = vacationTimeData.filter((row) => row.used_vacation_time > 0);

  // Update handlers to use mutations
  const handleTimesheetDataUpdate = (
    updater: (prevData: TimesheetReport[]) => TimesheetReport[]
  ) => {
    queryClient.setQueryData(['timesheetData'], (oldData: TimesheetReport[]) => {
  ...
  const handleFilteredDataUpdate = (filteredData: TimesheetReport[]) => {
    queryClient.setQueryData(['filteredTimesheetData'], (oldData: TimesheetReport[]) => {
  ...
  const handleDownload = () => {
    ...
    if (activeTab === 'vacation-time') {
      ...
      filteredVacationData.forEach((row) => {
        ...
        const totalUsedHours = row.hours_per_date.reduce((sum, hours) => sum + hours, 0);
      ...
      dataToExport = filteredTimesheetData.map((row) => ({
        Employee: row.name,
        Date: row.event_date
          ? formatTZ(toZonedTime(new Date(row.event_date), TIME_ZONE), 'M-dd-yyyy', {
              timeZone: TIME_ZONE,
            })
          : 'N/A',
  ...
  const handleCombinedDownload = () => {
    ...
    // Save the file
    const wbout = XLSX.write(wb, { bookType: 'xlsx', type: 'array' });
    saveAs(new Blob([wbout], { type: 'application/octet-stream' }), 'combined_time_report.xlsx');
  };
  ...
  const setDateRange = useMutation({
    mutationFn: (newDateRange: DateRange | undefined) => {
      return Promise.resolve(queryClient.setQueryData(['dateRange'], newDateRange));
    },
  });
          ...
          <div className="flex flex-col gap-4">
            <div className="flex justify-between items-center">
              ...
              <div className="flex gap-4">
                ...
                {dateRange && (
                  <Button variant="outline" onClick={() => setDateRange.mutate(undefined)}>
