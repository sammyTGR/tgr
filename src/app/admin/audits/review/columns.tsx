...
export const createColumns = (refreshData: () => void): ColumnDef<AuditData>[] => [
  {
    ...
    header: ({ column }) => <DataTableColumnHeader column={column} title="DROS Number" />,
    cell: ({ row }) => {
      ...
      return (
        <div className="flex space-x-2">
          ...
          <span className="max-w-[500px] truncate font-medium">{row.getValue('dros_number')}</span>
        </div>
      );
    },
    meta: {
  ...
  // },
  {
    ...
    cell: ({ row }) => {
      ...
      return (
        <div className="flex space-x-2">
          {label && <Badge variant="outline">{label.label}</Badge>}
          <span className="max-w-[500px] truncate font-medium">{formattedDate}</span>
        </div>
      );
    },
    meta: {
  ...
  {
    id: 'actions',
    cell: ({ row }) => <DataTableRowActions row={row} onAuditUpdated={refreshData} />,
  },
];
